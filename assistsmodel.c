/* nba_assists_model.c
 * A tunable model to project an NBA player's assists.
 *
 * Primary base:
 *   - Sportsbook assists line
 *   - Player season assists average
 *
 ...
 *   - Pace (matchup possessions)
 *   - Recent form (last N games AST vs season AST)
 *   - Minutes trend (expected vs season minutes)
 *   - Back-to-back penalty
 *   - Potential assists -> expected actual (conversion) vs season avg
 */
...
static const double W_POTENTIAL_AST    = 0.14;  /* potential->expected actual vs season avg */
...
typedef struct {
    ...
    double potential_ast;        /* player's potential assists per game (this matchup/expected) */
    double expected_conversion;  /* expected pass->make conversion rate, e.g., 0.55 */
} Inputs;
...
/* Potential assists:
 * - Convert potential assists into expected actual assists using expected_conversion (0..1).
 * - Compare to season avg AST to get a relative lift.
 */
static double m_potential_assists(const Inputs *in) {
    if (W_POTENTIAL_AST == 0.0 || in->season_avg_ast <= 0.0) return 1.0;
    double expected_actual = in->potential_ast * in->expected_conversion; /* e.g., 10 * 0.55 = 5.5 */
...
int main(void) {
    ...
    printf("Potential assists (expected this game): ");
    scanf("%lf", &in.potential_ast);

    printf("Expected conversion rate on potential assists (0–1, e.g., 0.54): ");
    scanf("%lf", &in.expected_conversion);
